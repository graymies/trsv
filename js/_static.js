            ...
            function adjust() {
                ...
                if (original !== height) {
                    if (options.animate && $ta.css('display') === 'block') {
                        $ta.stop().animate({
                            height: height
                        }, options.animateDuration, options.animateCallback);
                    } else {
...
var HighlanderComments = jQuery.extend(HighlanderComments, {
    ...
    init: function() {
        ...
        jQuery('div.comment-form-fields input').change(function() {
            HighlanderComments.toggleLabel(jQuery(this));
        }).focus(function() {
            HighlanderComments.hideLabels(jQuery(this));
        }).blur(function() {
            HighlanderComments.showLabels(jQuery(this));
        });
        jQuery('div.comment-form-fields label').click(function() {
            HighlanderComments.hideLabels(jQuery(this).parent().children('.comment-form-input input'));
        });
        ...
        jQuery('textarea#comment')
            ...
            .keyup(function() {
                HighlanderComments.fillShareText(jQuery(this));
            })
            .one('focus', function() {
        ...
        jQuery('#commentform').submit(function() {
            ...
            if ('' === jQuery('textarea#comment').val()) {
                ...
                jQuery('textarea#comment').focus(function() {
                    jQuery(this).parent('#comment-form-comment').removeClass('error');
                });
                verified = false;
            }
    ...
    moveForm: function(commId, parentId, respondId, postId) {
        ...
        if (HighlanderComments.currentParent) {
            ...
            if (rTop <= cTop) {
                ...
                respond.before(jQuery('<div id="highlander-placeholder" />').css({
                    margin: 0,
                    padding: 0,
                    border: 0,
                    height: rHeight,
                    visibility: 'hidden',
                    position: respond.css('position')
                }));
            }
    ...
    moveFormNow: function(commId, parentId, respondId) {
        ...
        var rgbaTransparent = /rgba.*,\s*0\s*\)/,
            cp, bg, bgi;
    ...
    toggleLabel: function(input) {
        ...
        if (!input.val().length) {
            input.parents('div.comment-form-field').children('label').css({
                opacity: 1
            });
        } else {
            input.parents('div.comment-form-field').children('label').css({
    ...
    hideLabels: function(input) {
        if (jQuery(window).width() == 0 || jQuery(window).width() > 450 && !input.attr('placeholder')) {
            input.parents('div.comment-form-field').children('label').animate({
                opacity: 0
            }, 150, function() {
                jQuery(this).hide();
            });
    ...
    showLabels: function(input) {
        if (jQuery(window).width() == 0 || jQuery(window).width() > 450 && !input.attr('placeholder')) {
            if (!input.val().length) {
                input.parents('div.comment-form-field').children('label').show().animate({
                    opacity: 1
                }, 150);
            }
        }
    ...
    toggleShareLabels: function() {
        if (jQuery('#respond').outerWidth() > 450) {
            jQuery('#comment-form-post-to label span').show().parent().css({
                'padding-right': 0
            });
        } else {
            jQuery('#comment-form-post-to label span').hide().parent().css({
                'padding-right': '20px'
            });
        }
        HighlanderComments.resizeCallback();
    ...
    fillShareText: function(textarea) {
        if (!jQuery('#comment-form-share-text-textarea').length) {
            return;
        }
